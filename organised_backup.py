...
_, _roi_gray = cv2.threshold(roi_gray, 50, 255, 0)  # white digits
...
vert_dilate3 = cv2.getStructuringElement(cv2.MORPH_RECT, ksize=(1, 2))
...
for _c, c in enumerate(digitCnts):
    ...
    if point is not None:
        digits.append('.')
        cv2.putText(output, str(digits[-1]), (x + 11, y + 25), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 0, 255),
                    1)  # (255, 0, 0)=red
        point = None
