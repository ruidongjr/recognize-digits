...
def read_digits(gray, image, skew=9, avg_digit_width=16, brightness=70, debug=False):
    ...
    digits = []
    point = None
    # todo compute avg width
    # w_dic = {}
    # for _c, c in enumerate(digitCnts):
    #     (x, y, w, h) = cv2.boundingRect(c)
    #     w_dic[w] = w_dic.get(w, 0) + 1
    # for k, v in w_dic.items():  # for name, age in dictionary.iteritems():  (for Python 2.x)
    #     if v == max(w_dic.values()):
    #         avg_digit_width = k
    ...
    for _c, c in enumerate(digitCnts):
        ...
        segments = [
            ((4, 0), ((w // 2) + 1, dH)),  # top
            ((1, 2), (dW, h // 2)),  # top-left
            ((w - dW, 2), (w - 1, h // 2)),  # top-right
            ((4, (h // 2) - dHC), ((w // 2) + 1, (h // 2) + dHC+1)),  # center
            ((1, (h // 2)-2), (dW, h-2)),  # bottom-left
            ((w - dW, (h // 2)-2), (w, h-2)),  # bottom-right
            ((4, h - dH+2), ((w // 2) + 1, h))  # bottom
        ]
        on = [0] * len(segments)
        if debug:
        ...
        for (i, ((xA, yA), (xB, yB))) in enumerate(segments):
            ...
            # if i in [1, 2, 4, 5]:  # vertical segments need high area due to kernel size (1, 3)
            #     if total / float(area) > 0.6:
            #         on[i] = 1
            # else:
            if area == 0:
                on[i] = 0
            elif total / float(area) > 0.5:
                on[i] = 1
...
if __name__ == '__main__':
    """
    this setting fits in front camera
    images test/f04-f09.jpg
    """
    path = 'test'
    image = join(path, 'f09.jpg')

    start = time()
    image = cv2.imread(image)

    image = cv2.resize(image, (128, 48), interpolation=cv2.INTER_AREA)
    # cv2.imwrite('test/02_.jpg', image)
    ...
    show_image = False
    digits = read_digits(gray, image, skew=8, avg_digit_width=13, brightness=70, debug=show_image)
    # assert number -1
    if -1 in digits:
